...
public class UserDao {
	...
	public int save(User user) {
		return jdbcTemplate.update(
				"INSERT INTO `quiz`.`user` ( `first_name`, `last_name`, `email_id`, `password`, `mobile_number`) "
						+ " VALUES ( ?, ?, ?, ?, ?)",
	...
	public int updatePassword(Long id, String newPassword) {
		return jdbcTemplate.update("UPDATE `quiz`.`user` SET `password` = ?, `updated_on` = now() WHERE `id` = ?",
